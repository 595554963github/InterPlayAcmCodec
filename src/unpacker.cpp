...
void CValueUnpacker::prepare_bits (int bits) {
	while (bits > avail_bits) {
		...
		if (in_buf_pos >= in_buf_len && !in_buf_eof) {
			// refill the read-ahead buffer; the old code checked the stream
			// boundary per byte with ftell(), which cost a locked CRT call
			// for every single input byte
			long remain = (long) maxlen - ftell (file);
			if (remain > 0) {
				int want = (remain < (long) IN_BUF_SIZE) ? (int) remain : IN_BUF_SIZE;
				in_buf_len = (int) fread (in_buf, 1, want, file);
				in_buf_pos = 0;
				if (in_buf_len <= 0) {
					in_buf_len = 0;
					in_buf_eof = 1;
		...
		if (!in_buf_eof && in_buf_pos < in_buf_len) {
			one_byte = in_buf [in_buf_pos++];
		} else {
			// past the stream boundary (or a short file): same as the old
			// fread-failure path, feed zero bits
			one_byte = 0;
		}
...
//Rus: использует??, когд? весь столбе? ? номеро? pass заполнен ну??ми
...
//Rus: cтолбец pass заполнен ну??ми, ? такж? +/- 1, но нули част? идут подр??
...
//Rus: имее? смыс? использовать, когд? веро?тность парных нуле? (p00) больше 2/3
	for (int i=0; i<subblocks; i++) {
		prepare_bits (3);
		if ((next_bits & 1) == 0) {
			avail_bits--;
...
//Rus: cтолбец pass заполнен ну??ми, ? такж? +/- 1
// efficiency: 2-P0. P0 - cnt of any zero (P0 = p0 + p00)
//Eng: use it when P0 > 1/3
//Rus: имее? смыс? использовать, когд? веро?тность ну?? больше 1/3
	for (int i=0; i<subblocks; i++) {
		prepare_bits (2);
		if ((next_bits & 1) == 0) {
			avail_bits--;
...
//Rus: вс? комбинации трое? -2, -1, 0, +1, 2.
// эффективност?: 7/3 бита на значение - всегда
// use it when p0 <= 1/3
	for (int i=0; i<subblocks; i++) {
...
//Rus: вс? комбинации па? от -5 до +5
// эффективност?: 7/2 бита на значение - всегда
	for (int i=0; i<subblocks; i++) {
		char bits = (char) (get_bits (7) & 0x7f);
