...
int CSubbandDecoder::init_decoder() {
	int memory_size = (levels == 0) ? 0 : (4 * (block_size >> 1) - 2);
	if (memory_size) {
...
void CSubbandDecoder::decode_data(int32_t* buffer, int blocks) {
	...
	int sb_size = block_size >> 1;

	blocks <<= 1;
	sub_4d3fcc(mem_ptr, buff_ptr, sb_size, blocks);
	mem_ptr += sb_size * 2;
...
void CSubbandDecoder::sub_4d3fcc(int32_t* memory, int32_t* buffer, int sb_size, int blocks) {
